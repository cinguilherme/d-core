...
(defprotocol StateStoreProtocol
  ...
  (set-max-field! [this key field value opts] "Set field only when value is greater than current numeric value. Value must be numeric (number or numeric string); invalid input should fail fast. When :ttl-ms is provided by Redis-backed implementations, TTL may be refreshed even when the value is not updated.")
